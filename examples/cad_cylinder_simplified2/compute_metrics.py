...
def transform(obj: trimesh.Trimesh) -> trimesh.Trimesh:
    ...
    return center_mesh(
        obj.apply_transform(trimesh.transformations.translation_matrix([0, 0, 0]))
    )
    return center_mesh(obj)


def align_rot(mesh_a, mesh_b):
    ...
    if np.linalg.det(R_opt) < 0:
        ...
        error_original = np.linalg.norm(A @ R_opt.T - B)
        error_reflect = np.linalg.norm(A @ R_reflect.T - B)
