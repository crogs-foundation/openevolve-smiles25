...
def load_dataset(data_file_path):
    ...
    try:
        ...
        for review in data.get("book_reviews", []):
            reviews.append({"text": review["text"], "label": review["sentiment_score"]})
...
def evaluate(prompt_path):
    ...
    try:
        ...
        try:
            test_model = OpenAI(base_url=TASK_MODEL_URL, api_key=TASK_MODEL_API_KEY)
        ...
        print(
            f"Using {len(ds_sample)} samples from {len(ds)} total reviews for evaluation"
        )
        ...
        prompt_match = re.search(
            r"EVOLVE-BLOCK-START(.*)EVOLVE-BLOCK-END", prompt, re.DOTALL
        )
        if prompt_match:
            ...
            for attempt in range(max_retries):
                try:
                    response = test_model.chat.completions.create(
                        model=TASK_MODEL_NAME, messages=messages
                    )
                    break
                except Exception as e:
        ...
        if len(individual_scores) > 1:
            ...
            variance = sum((x - mean) ** 2 for x in individual_scores) / len(
                individual_scores
            )
        ...
        high_accuracy_rate = (
            high_accuracy_count / len(individual_scores) if individual_scores else 0.0
        )
